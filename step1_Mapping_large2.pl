...
Run shell (Use less memory, so it is easy to run/ but java sometimes make errors, need to split large fq files and check the work0001.sh.e* if killed):
...
foreach my $file (keys %$fqlist){
        ...
        `perl /rhome/cjinfeng/software/bin/qsub-pbs.pl --maxjob 20 --lines 2 --interval 120 --resource walltime=100:00:00 --convert no $fqhead.clean.sh`;
