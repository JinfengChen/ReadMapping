...
if (exists $opt{1} and exists $opt{2}){
   if ($opt{tool}=~/bwa/){
      ...
      my $fqhead2;
      if ($opt{1}=~/\.gz$/){
         $fqhead1= $opt{1}=~/fastq$/ ? basename($opt{1},".fastq.gz") : basename($opt{1},".fq.gz");
         $fqhead2= $opt{2}=~/fastq$/ ? basename($opt{2},".fastq.gz") : basename($opt{2},".fq.gz");
      }else{
